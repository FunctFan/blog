...
# 192.168.0.0 是你的 OpenVPN 服务器所在在局域网的网段
...
虽然你的 OpenVPN 服务已经正常启动了，但是你会发现你还是无法连接它，因为外界还无法跟你的 OpenVPN 服务器通信，路由器默认是会拦截并丢弃
外侧网络发起的主动连接，也就是说路由器默认是只能让你主动出去，而不允许网面的主动请求进来，原因很简单，这样能保证你内部局域网主机的安全。
路由器两端连接的是不同的网络，而不同网络之间是不能相互通信的。因为对于外部的主机来说，我们内部局域网所有主机都是共享一个公网 IP，比如为 
14.153.76.90，那么此时外部主机向 14.153.76.90 发送一条消息，路由器改怎么处理呢，因为此时路由器并不知道你这条消息要发给谁，只能把这个消息
包丢弃。
